...
export async function getRecentTracks(ctx: HttpContext): Promise<void> {
  const userId = getAuthUserId(ctx);
  const list = await trackService.getRecentVisits(userId);
  ctx.body = success(list);
}

export async function listMyTracks(ctx: HttpContext): Promise<void> {
  const userId = getAuthUserId(ctx);
...
export async function getTrackById(ctx: HttpContext): Promise<void> {
  const { id } = ctx.params as { id: string };
  const detail = await trackService.getById(id, ctx.state.auth?.userId);
  ctx.body = success(detail);
}

export async function createTrack(ctx: HttpContext): Promise<void> {
  const userId = getAuthUserId(ctx);
...
export async function updateTrack(ctx: HttpContext): Promise<void> {
  const userId = getAuthUserId(ctx);
  const { id } = ctx.params as { id: string };
...
export async function touchRecentVisit(ctx: HttpContext): Promise<void> {
  const userId = getAuthUserId(ctx);
  const { trackId } = ctx.params as { trackId: string };
  await trackService.touchRecentVisit(userId, trackId);
