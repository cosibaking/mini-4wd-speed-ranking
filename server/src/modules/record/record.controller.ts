...
export async function submitRecord(ctx: Context): Promise<void> {
...
export async function getRecordById(ctx: Context): Promise<void> {
  const { id } = ctx.params as { id: string };
  const detail = await recordService.getById(id);
  ctx.body = success(detail);
}

export async function listMyRecords(ctx: Context): Promise<void> {
  const userId = getAuthUserId(ctx);
